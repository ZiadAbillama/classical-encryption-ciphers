...
import { Trophy, Star, Key, Sparkles, Grid3x3, Zap, CheckCircle2, Sun, Moon } from 'lucide-react';
...
const MissionsPage = () => {
  ...
  // Safe stats object so we never crash if user.stats is missing
  const stats = {
    points: 0,
    level: 1,
    combo: 0,
    ...
    <div className={`min-h-screen ${currentTheme.bg} relative overflow-hidden`}>
      ...
      <header className={`relative backdrop-blur-xl ${currentTheme.header} border-b ${currentTheme.cardBorder}`}>
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className={`flex items-center space-x-2 px-4 py-2 ${theme === 'dark' ? 'bg-purple-500/20 border-purple-500/40' : 'bg-slate-100 border-slate-300'} border rounded-full`}>
              <Trophy className={`w-4 h-4 ${theme === 'dark' ? 'text-amber-400' : 'text-amber-600'}`} />
              <span className={`${currentTheme.text} font-bold`}>{stats.points} pts</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="relative max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className={`backdrop-blur-xl ${currentTheme.card} border ${currentTheme.cardBorder} rounded-2xl p-6 text-center shadow-lg`}>
            <div className={`text-4xl font-black mb-2 ${theme === 'dark' ? 'bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent' : 'text-slate-700'}`}>
              {stats.completedChallenges.length}
            </div>
            <div className={`${currentTheme.textMuted} text-sm`}>Completed</div>
          </div>
          <div className={`backdrop-blur-xl ${currentTheme.card} border ${currentTheme.cardBorder} rounded-2xl p-6 text-center shadow-lg`}>
            <div className={`text-4xl font-black mb-2 ${theme === 'dark' ? 'bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent' : 'text-slate-700'}`}>
              {missions.length - stats.completedChallenges.length}
            </div>
            <div className={`${currentTheme.textMuted} text-sm`}>Remaining</div>
          </div>
          <div className={`backdrop-blur-xl ${currentTheme.card} border ${currentTheme.cardBorder} rounded-2xl p-6 text-center shadow-lg`}>
            <div className={`text-4xl font-black mb-2 ${theme === 'dark' ? 'bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent' : 'text-slate-700'}`}>
              {missions.reduce(
                (sum, m) => stats.completedChallenges.includes(m.id) ? sum + m.points : sum,
                0
              )}
            </div>
            <div className={`${currentTheme.textMuted} text-sm`}>Points Earned</div>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {missions.map(mission => {
            const isCompleted = stats.completedChallenges.includes(mission.id);
              ...
              >
                {isCompleted && (
                  <div className="absolute -top-3 -right-3">
                    <div className={`${theme === 'dark' ? 'bg-gradient-to-r from-green-600 to-teal-600' : 'bg-gradient-to-r from-green-500 to-emerald-500'} rounded-full p-2 border-4 ${theme === 'dark' ? 'border-slate-900' : 'border-white'} shadow-xl`}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`${theme === 'dark' ? 'text-amber-400' : 'text-amber-600'} font-bold`}>{mission.points} pts</span>
                  </div>
                  {!isCompleted && (
                    <button
        ...
        <div className={`mt-12 backdrop-blur-xl ${currentTheme.card} border ${currentTheme.cardBorder} rounded-2xl p-8 shadow-lg`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-xl font-bold ${currentTheme.text}`}>Overall Progress</h3>
            <span className={`${currentTheme.text} font-bold text-lg`}>
              {stats.completedChallenges.length} / {missions.length}
            </span>
          </div>
          <div className={`relative w-full h-6 ${theme === 'dark' ? 'bg-slate-700' : 'bg-slate-200'} rounded-full overflow-hidden`}>
            <div
              className={`absolute top-0 left-0 h-full ${theme === 'dark' ? 'bg-gradient-to-r from-purple-600 via-pink-600 to-red-600' : 'bg-gradient-to-r from-slate-600 via-slate-700 to-slate-800'} rounded-full transition-all duration-500`}
              style={{ width: `${(stats.completedChallenges.length / missions.length) * 100}%` }}
            ></div>
          </div>
          <p className={`${currentTheme.textMuted} text-sm mt-2 text-center`}>
            {stats.completedChallenges.length === missions.length 
              ? '🎉 All missions completed! You are a Crypto Master!' 
              : `Complete ${missions.length - stats.completedChallenges.length} more missions to become a Crypto Master!`
            }
          </p>
        </div>
      </div>
