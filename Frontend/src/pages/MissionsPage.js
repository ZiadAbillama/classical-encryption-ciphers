    ...
    <div className={`min-h-screen ${currentTheme.bg} relative overflow-hidden`}>
        ...
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className={`flex items-center space-x-2 px-4 py-2 ${theme === 'dark' ? 'bg-purple-500/20 border-purple-500/40' : 'bg-slate-100 border-slate-300'} border rounded-full`}>
              ...
              <span className={`${currentTheme.text} font-bold`}>{user.stats.points} pts</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="relative max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <div className={`backdrop-blur-xl ${currentTheme.card} border ${currentTheme.cardBorder} rounded-2xl p-6 text-center shadow-lg`}>
            <div className={`text-4xl font-black mb-2 ${theme === 'dark' ? 'bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent' : 'text-slate-700'}`}>
              {missions.reduce((sum, m) => user.stats.completedChallenges.includes(m.id) ? sum + m.points : sum, 0)}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {missions.map(mission => {
            const isCompleted = user.stats.completedChallenges.includes(mission.id);
        ...
        <div className={`mt-12 backdrop-blur-xl ${currentTheme.card} border ${currentTheme.cardBorder} rounded-2xl p-8 shadow-lg`}>
          ...
          <div className={`relative w-full h-6 ${theme === 'dark' ? 'bg-slate-700' : 'bg-slate-200'} rounded-full overflow-hidden`}>
            <div
              ...
              style={{ width: `${(user.stats.completedChallenges.length / missions.length) * 100}%` }}
            ></div>
          </div>
          <p className={`${currentTheme.textMuted} text-sm mt-2 text-center`}>
            {user.stats.completedChallenges.length === missions.length 
              ? '🎉 All missions completed! You are a Crypto Master!' 
              : `Complete ${missions.length - user.stats.completedChallenges.length} more missions to become a Crypto Master!`
            }
          </p>
        </div>
      </div>
